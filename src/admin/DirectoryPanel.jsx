...
import { SectionHeader, Loading, ErrorNote, EmptyState, ConfirmDialog, relativeTime, wideDialog } from './ui'
  ...
  return (
    <Box>
      ...
      <Box sx={{ bgcolor: '#fff', border: '1px solid', borderColor: 'divider', borderRadius: 3, overflow: 'hidden' }}>
        {tab === 'members' ? (
          members.length === 0 ? <EmptyState>No directory entries.</EmptyState> : members.map((m, i) => (
            <Row
              key={m.id} first={!i}
...
function Row({ first, name, secondary, photo, meta, trailing, onEdit, onDelete }) {
  return (
